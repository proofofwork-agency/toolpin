...
Browse rows show public verdicts:
  VERIFIED     pinned target plus ToolPin-verified npm/OCI/MCPB proof
  NEEDS REVIEW useful metadata, but artifact proof is missing/stale/declared/weak/failed
...
Overview top rows are registry metadata summary. Details still show the internal
tier, metadata profile score, pillar scores, and cap reason.`);
}

export function interactiveHelp(): void {
  console.log(`Usage: toolpin interactive [query] [--source id|all] [--live] [--limit 10] [--client ${CLIENT_USAGE}] [--scope project|global] [--version <server-version>] [--verify] [--require-verified] [--timeout 15000] [--policy .toolpin/policy.json] [--no-policy] [--no-input] [--explain] [--color auto|always|never]
...
--explain includes internal tier/profile/evidence details in review guidance.`);
}
