...
- **Artifact verification is still syntax-oriented, not byte-level.** OCI identifiers must carry a valid-looking `@sha256:<64 hex>` digest and MCPB `fileSha256` must be 64 hex characters, but ToolPin still does not fetch/recompute bytes. These declared pins no longer earn the `verified` tier by themselves. Attestations are *declared*, never verified (`isAttestation` only checks `typeof type === "string"`). The badge `sigstore-declared` is honest naming, but users may still overread it.
