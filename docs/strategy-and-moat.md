...
- **Cryptographically sound lockfile integrity model.** Per-entry SHA-256 over stable JSON for reviewed entry contents, including entry timestamps; separate whole-lock digest that excludes only top-level file timestamps; detached Ed25519 signatures over the digest. `signedAt` is correctly excluded from signed bytes, while normalized tool-description hashes are included when present.
...
- **Artifact verification is still partial.** OCI identifiers must carry a valid `@sha256:<64 hex>` digest and are best-effort checked against registry manifest digests when reachable. MCPB `fileSha256` must be 64 hex characters and is recomputed only from allowlisted HTTPS artifact hosts. npm packages are checked against `registry.npmjs.org` packument `dist.integrity` and trusted npm tarball bytes. PyPI/NuGet/Cargo artifact integrity, OCI image byte recomputation, and sigstore/cosign verification are still not implemented. Declared pins no longer earn the `verified` tier by themselves. Attestations are *declared*, never verified (`isAttestation` only checks `typeof type === "string"`). The badge `sigstore-declared` is honest naming, but users may still overread it.
